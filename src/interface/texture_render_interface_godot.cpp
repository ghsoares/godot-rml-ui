#include "texture_render_interface_godot.h"
...
Rml::Matrix4f TextureRenderInterfaceGodot::get_final_transform(const Rml::Matrix4f &p_drawing_matrix, const Rml::Vector2f &translation) {
    return Rml::Matrix4f::Translate(Rml::Vector3f(translation.x, translation.y, 0.0)) * p_drawing_matrix;
}

void TextureRenderInterfaceGodot::matrix_to_pointer(float *ptr, const Rml::Matrix4f &p_mat) {
    ptr[0] = p_mat[0].x;
    ptr[1] = p_mat[0].y;
    ptr[2] = p_mat[0].z;
    ptr[3] = p_mat[0].w;
...
RID *TextureRenderInterfaceGodot::filter_get_texture(RenderFrame *frame, const BufferTarget &target) {
	switch (target) {
...
RID *TextureRenderInterfaceGodot::filter_get_framebuffer(RenderFrame *frame, const BufferTarget &target) {
	switch (target) {
...
void TextureRenderInterfaceGodot::create_shader_pipeline(uint64_t p_id, const std::map<String, Variant> &p_params) {
...
RID TextureRenderInterfaceGodot::get_shader_pipeline(uint64_t p_id) const {
    return clip_mask_enabled ? std::get<1>(pipelines.at(p_id)) : std::get<0>(pipelines.at(p_id));
}

void TextureRenderInterfaceGodot::initialize() {
...
void TextureRenderInterfaceGodot::finalize() {
    internal_rendering_resources.free_all_resources();
}

void TextureRenderInterfaceGodot::allocate_render_target(RenderTarget *p_target) {
    RenderingServer *rs = RenderingServer::get_singleton();
...
void TextureRenderInterfaceGodot::free_render_target(RenderTarget *p_target) {
...
void TextureRenderInterfaceGodot::allocate_render_frame(RenderFrame *p_frame) {
    if (p_frame->current_size == p_frame->desired_size) {
        return;
    }

    p_frame->current_size = p_frame->desired_size;

    RenderingServer *rs = RenderingServer::get_singleton();

    free_render_frame(p_frame);

    p_frame->clip_mask = rendering_resources.create_texture({
        {"width", p_frame->current_size.x},
        {"height", p_frame->current_size.y},
    ...
    p_frame->clip_mask_framebuffer = rendering_resources.create_framebuffer({
        {"textures", TypedArray<RID>({ p_frame->clip_mask })}
    });

    allocate_render_target(&p_frame->main_target);
    allocate_render_target(&p_frame->primary_filter_target);
    allocate_render_target(&p_frame->secondary_filter_target);
    allocate_render_target(&p_frame->blend_mask_target);
    p_frame->main_tex = rs->texture_rd_create(p_frame->main_target.color0);
}

void TextureRenderInterfaceGodot::free_render_frame(RenderFrame *p_frame) {
    RenderingServer *rs = RenderingServer::get_singleton();
...
void TextureRenderInterfaceGodot::blit_texture(const RID &p_dst, const RID &p_src, const Vector2i &p_dst_pos, const Vector2i &p_src_pos, const Vector2i &p_size) {
...
void TextureRenderInterfaceGodot::push_context(void *&p_ctx, const Vector2i &p_size) {
    RenderFrame *frame = static_cast<RenderFrame *>(p_ctx);
    if (frame == nullptr) {
        frame = memnew(RenderFrame);
        p_ctx = frame;
    }
    frame->desired_size = p_size;
    target_frame = frame;

    allocate_render_frame(frame);

    render_target_stack.push_back(&frame->main_target);
    RD *rd = rendering_resources.device();
    rd->texture_clear(frame->main_target.color0, Color(0, 0, 0, 0), 0, 1, 0, 1);
    rd->texture_clear(frame->main_target.color1, Color(0, 0, 0, 0), 0, 1, 0, 1);
}

void TextureRenderInterfaceGodot::pop_context() {
...
void TextureRenderInterfaceGodot::draw_context(void *&p_ctx, const RID &p_canvas_item) {
    RenderFrame *frame = static_cast<RenderFrame *>(p_ctx);
    RenderingServer *rs = RenderingServer::get_singleton();

    Vector2i size = frame->current_size;
...
void TextureRenderInterfaceGodot::free_context(void *&p_ctx) {
    RenderFrame *frame = static_cast<RenderFrame *>(p_ctx);
...
Rml::CompiledGeometryHandle TextureRenderInterfaceGodot::CompileGeometry(Rml::Span<const Rml::Vertex> vertices, Rml::Span<const int> indices) {
...
void TextureRenderInterfaceGodot::RenderGeometry(Rml::CompiledGeometryHandle geometry, Rml::Vector2f translation, Rml::TextureHandle texture) {
...
void TextureRenderInterfaceGodot::ReleaseGeometry(Rml::CompiledGeometryHandle geometry) {
    MeshData *mesh_data = reinterpret_cast<MeshData *>(geometry);
    ERR_FAIL_NULL(mesh_data);
...
Rml::TextureHandle TextureRenderInterfaceGodot::LoadTexture(Rml::Vector2i& texture_dimensions, const Rml::String& source) {
...
Rml::TextureHandle TextureRenderInterfaceGodot::GenerateTexture(Rml::Span<const Rml::byte> source, Rml::Vector2i source_dimensions) {
...
void TextureRenderInterfaceGodot::ReleaseTexture(Rml::TextureHandle texture) {
    TextureData *tex_data = reinterpret_cast<TextureData *>(texture);
...
void TextureRenderInterfaceGodot::EnableScissorRegion(bool enable) {
    scissor_enabled = enable;
}

void TextureRenderInterfaceGodot::SetScissorRegion(Rml::Rectanglei region) {
...
void TextureRenderInterfaceGodot::EnableClipMask(bool enable) {
	clip_mask_enabled = enable;
}

void TextureRenderInterfaceGodot::RenderToClipMask(Rml::ClipMaskOperation operation, Rml::CompiledGeometryHandle geometry, Rml::Vector2f translation) {
    ERR_FAIL_COND(render_target_stack.size() == 0);
    RenderTarget *target = render_target_stack[render_target_stack_ptr];
...
void TextureRenderInterfaceGodot::SetTransform(const Rml::Matrix4f* transform) {
...
Rml::LayerHandle TextureRenderInterfaceGodot::PushLayer() {
...
void TextureRenderInterfaceGodot::RenderFilter(Rml::CompiledFilterHandle filter) {
...
void TextureRenderInterfaceGodot::CompositeLayers(Rml::LayerHandle source, Rml::LayerHandle destination, Rml::BlendMode blend_mode, Rml::Span<const Rml::CompiledFilterHandle> filters) {
...
void TextureRenderInterfaceGodot::PopLayer() {
    render_target_stack_ptr--;
}

Rml::TextureHandle TextureRenderInterfaceGodot::SaveLayerAsTexture() {
    RenderTarget *target = render_target_stack[render_target_stack_ptr];
...
Rml::CompiledFilterHandle TextureRenderInterfaceGodot::SaveLayerAsMaskImage() {
    RenderTarget *target = render_target_stack[render_target_stack_ptr];
...
Rml::CompiledFilterHandle TextureRenderInterfaceGodot::CompileFilter(const Rml::String& name, const Rml::Dictionary& parameters) {
    ShaderRenderData params;
...
void TextureRenderInterfaceGodot::ReleaseFilter(Rml::CompiledFilterHandle filter) {
    ShaderRenderData *shader_data = reinterpret_cast<ShaderRenderData *>(filter);

    memdelete(shader_data);
}

Rml::CompiledShaderHandle TextureRenderInterfaceGodot::CompileShader(const Rml::String& name, const Rml::Dictionary& parameters) {
    ShaderRenderData params;
    params.name = rml_to_godot_string(name);
...
void TextureRenderInterfaceGodot::RenderShader(Rml::CompiledShaderHandle shader, Rml::CompiledGeometryHandle geometry, Rml::Vector2f translation, Rml::TextureHandle texture) {
    ERR_FAIL_COND(render_target_stack.size() == 0);
    RenderTarget *target = render_target_stack[render_target_stack_ptr];
...
void TextureRenderInterfaceGodot::ReleaseShader(Rml::CompiledShaderHandle shader) {
    ShaderRenderData *shader_data = reinterpret_cast<ShaderRenderData *>(shader);
